// // src/pages/admin/AllMenteeSupportTickets.jsx
...
//   // ─── API ───────────────────────────────────────────────────────────────────
//   const { data, isLoading, error, refetch } = useGetAllMenteeSupportTicketsQuery();
//   const [updateTicket, { isLoading: isUpdating }] = useUpdateMenteeSupportTicketMutation();
//   const [respondToTicket, { isLoading: isResponding }] = useRespondToMenteeTicketMutation();

//   // ─── Clear state only AFTER Bootstrap finishes hiding the modal ────────────
//   useEffect(() => {
//     const modalEl = document.getElementById("menteeResponseModal");
//     if (!modalEl) return;
...
//         t.menteeId?.name?.toLowerCase().includes(q) ||
//         t.menteeId?.email?.toLowerCase().includes(q)
//       )
//     );
//   });
...
//                 <h1 className="mb-3">Mentee Support Tickets</h1>
...
//                           {["Technical Issue", "Session Management", "Mentor Related",
...
//                       <p className="mt-3">Loading mentee support tickets...</p>
//                     </div>
//                   ) : error ? (
...
//                               <td><div className="fw-semibold small">{ticket.name || "N/A"}</div></td>
//                               <td>
...
//                                   data-bs-target="#menteeResponseModal"
//                                   title="View / Respond"
//                                 >
...
//                               <p className="mb-0">No mentee support tickets found</p>
//                             </td>
//                           </tr>
//                         )}
//                       </tbody>
...
//                       <div className="mb-3">
//                         <div className="form__box">
//                           <label><Icon icon="mdi:comment-account" width="18" className="me-1" />Mentee Remarks</label>
...
//                           placeholder="Enter your response to the mentee..."
//                         />
//                         <small className="text-muted">Provide a detailed response to help resolve the mentee's query.</small>
//                       </div>
//                     </div>
...
// export default AllMenteeSupportTickets;
...
const AllMenteeSupportTickets = () => {
    ...
    const all = data?.tickets || [];
    const statsCards = [
        { label: "Total Tickets", count: all.length },
        { label: "Pending", count: all.filter((t) => t.status === "pending").length },
        { label: "In Progress", count: all.filter((t) => t.status === "in_progress").length },
        { label: "Resolved", count: all.filter((t) => t.status === "resolved").length },
    ];
    ...
    // ── FIXED: using ticket.ticketId not ticket._id ──────────────────────────
    const handleQuickStatusUpdate = async (ticketId, newStatus) => {
        try {
            await updateTicket({ ticketId, updates: { status: newStatus } }).unwrap();
            toast.success(`Status updated to ${newStatus.replace("_", " ")}`);
    ...
    const handleSubmitResponse = async () => {
        ...
        try {
            const admin = JSON.parse(localStorage.getItem("adminData") || "{}");
            const respondedBy = admin._id || admin.id || "Support Team";

            await respondToTicket({
                ticketId: ticket.ticketId,   // ← FIXED: was ticket._id
                response,
                respondedBy
            }).unwrap();
    ...
    return (
      ...
      `}</style>
            ...
            <section className="profile_section user__management py-4">
                                                ...
                                                {paginated.length ? (
                                                    paginated.map((ticket, i) => (
                                                        <tr key={ticket._id}>
                                                            <td>{startIdx + i + 1}</td>
                                                            <td><span className="badge">{ticket.ticketId}</span></td>
                                                            <td><div className="fw-semibold small">{ticket.name || ticket.username || "N/A"}</div></td>
                                                            <td>
                                                                <div className="text-truncate" style={{ maxWidth: "220px" }} title={ticket.subject}>
                                                                    {ticket.subject}
                                                                </div>
                                                            ...
                                                            <td>
                                                                {/* ── FIXED: passing ticket.ticketId not ticket._id ── */}
                                                                <select
                                                                    className="form-select form-select-sm shadow-none"
                                                                    style={{ minWidth: "120px" }}
                                                                    value={ticket.status}
                ...
                {/* Response Modal */}
                <div className="modal fade common__modal" id="menteeResponseModal" tabIndex="-1" aria-hidden="true">
                    <div className="modal-dialog modal-dialog-centered modal-lg">
                        <div className="modal-content">
                            <div className="modal-header border-0 pb-0">
                                <button type="button" className="btn-close" data-bs-dismiss="modal" aria-label="Close" />
                            ...
                            <div className="modal-body pt-0 px-4">
                                {selectedTicket && (
                                    <>
                                        ...
                                        <div className="row mb-3">
                                            {[
                                                { label: "Mentee Name", value: selectedTicket.name || selectedTicket.username },
                                                { label: "Category", value: selectedTicket.category },
                                                { label: "Priority", value: selectedTicket.priority?.toUpperCase() },
                                                { label: "Status", value: selectedTicket.status?.replace("_", " ").toUpperCase() },
                                                { label: "Created At", value: formatDate(selectedTicket.createdAt) },
                                        ...
                                        <div className="text-center my-4 d-flex justify-content-center gap-3">
                                            <button type="button" className="btn btn-secondary px-4" data-bs-dismiss="modal">
                                                <Icon icon="mdi:close" width="18" className="me-1" />Close
                                            </button>
                                            <button
                                                type="button"
                                                className="btn btn-primary px-4"
