...
const Userinfo = () => {
  ...
  const createSummarySection = (doc, user, directReferrals, chainReferrals) => {
    ...
    doc.text(`Jaimax Referral Report for ${user.name}`, 105, 20, {
      align: "center",
    });
    ...
    doc.text(
      `Generated by Dev-Team On: ${new Date().toLocaleString()}`,
      105,
      30,
      {
        align: "center",
    ...
    doc.setFont("helvetica", "normal");
    doc.text(
      `Note: This report have only First Three Investments and First Three Purchases`,
      105,
      35,
      { align: "center" }
    );
    ...
    doc.setFontSize(14);
    doc.setFont("helvetica", "bold");
    doc.text("Referral Statistics", 20, 95);
  ...
  const createUserDetailsSection = (doc, user) => {
    ...
    const userInfo = [
      ...
      ["Referral Amount", user.referenceInr.toFixed(2)],
      ["Referrer ID", user.referenceId],
      ["Tokens", user.tokens],
      ["Inr", user.Inr],
      ["Referral Count", user.referenceCount],
      ["Status", user.isActive ? "Active" : "Inactive"],
      ["Account Created On", new Date(user.createdAt).toLocaleString()],
      ["Wallet Amount", user.walletBalance.toFixed(2)],
      ["Verified", user.isVerified ? "Verified" : "Not Verified"],
      ["KYC Status", user.isVerified ? "Verified" : "Pending"],
    ];
